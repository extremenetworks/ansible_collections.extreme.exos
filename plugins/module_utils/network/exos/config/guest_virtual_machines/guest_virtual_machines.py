...
from ansible_collections.extreme.exos.plugins.module_utils.network.exos.utils.utils import search_obj_in_list
                ...
                for vport in h["virtual_ports"]:
                    if vport["type"] == "vtd":
                        if ((wvport["type"] == "vtd" and str(wvport["name"]) == str(vport["port"]))
                                or (wvport["type"] == "sriov" and str(wvport["port"]) == str(vport["port"]))):
